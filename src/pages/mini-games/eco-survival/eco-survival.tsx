...
type Task =
  | { kind: 'chain'; prompt: string; chain: string[] }
...
const LEVELS: LevelSpec[] = [
  { id: 1, title: 'Level 1 – Forest', biome: 'forest', tasks: [
  ...
  { id: 2, title: 'Level 2 – Desert', biome: 'desert', tasks: [
  ...
  { id: 3, title: 'Level 3 – Ocean', biome: 'ocean', tasks: [
  ...
  { id: 4, title: 'Level 4 – Tundra', biome: 'tundra', tasks: [
  ...
  { id: 5, title: 'Level 5 – Balance in Nature', biome: 'balance', tasks: [
...
const EcoSurvival: React.FC<EcoSurvivalProps> = ({ onBack }) => {
  ...
  useEffect(() => { setTaskIndex(0); setSelected(null); setInstant(null); setChainPick([]) }, [level])

  const currentTask = spec.tasks[taskIndex]

  const onSelectOption = (i: number) => { if (currentTask.kind === 'mcq') { setSelected(i); setInstant(i === currentTask.correctIndex ? 'correct' : 'wrong') } }
  const onPickChainItem = (i: number) => { if (currentTask.kind !== 'chain') return; setChainPick(p => p.includes(i) ? p.filter(x => x!==i) : [...p, i]) }

  const checkTask = () => {
    if (currentTask.kind === 'mcq') {
      if (selected == null) return
      const ok = selected === currentTask.correctIndex
      if (!ok) { setInstant('wrong'); setHealth(h => { const left = Math.max(0, h - 1); if (left===0) setStatus('gameOver'); return left }) ; return }
      setInstant('correct'); setEnergy(e => e + 20)
    } else {
      ...
      if (!ok) { setHealth(h => { const left = Math.max(0, h - 1); if (left===0) setStatus('gameOver'); return left }); return }
      setEnergy(e => e + 20)
    }

    if (taskIndex + 1 >= 3) { if (level >= LEVELS.length) setStatus('gameComplete'); else setStatus('levelComplete') }
    else { setTaskIndex(i => i + 1); setSelected(null); setInstant(null); setChainPick([]) }
  }
          ...
          {currentTask.kind === 'mcq' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              {currentTask.options.map((opt, i) => (
                <button key={i} onClick={() => onSelectOption(i)} className={`px-4 py-3 rounded-lg border ${selected===i ? 'bg-indigo-500/30 border-indigo-300' : 'bg-white/10 border-white/20 hover:bg-white/20'}`}>
                  {opt}
                </button>
              ))}
            </div>
          ...
          {currentTask.kind === 'chain' && (
            <div className="flex flex-wrap gap-3">
              {currentTask.chain.map((item, i) => {
                ...
                return (
                  <button key={i} onClick={() => onPickChainItem(i)} className={`px-4 py-3 rounded-lg border relative ${pickedIdx>=0 ? 'bg-emerald-500/30 border-emerald-300' : 'bg-white/10 border-white/20 hover:bg-white/20'}`}>
                    {item}
