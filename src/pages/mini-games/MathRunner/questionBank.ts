...
// --- Grade 8 Mathematics dataset (provided) ---
const G8_TOPICS: Record<string, string[]> = {
  'Fractions': [
    'What is 1/2 + 1/4?',
    'What is 2/3 of 12?',
    ...
    'Which is smallest: 1/2, 1/3, or 1/4?',
    'What is 4/5 + 1/10?',
    'Divide 3/4 by 1/2',
  ],
  'Algebra Basics': [
    ...
    'What is 2x when x = 4?',
    'Solve: x - 7 = 12',
    'What is 3x + 2 when x = 2?',
    'Solve: 4x = 20',
    'If a = b + 3 and b = 5, what is a?',
    'Solve: x/3 = 4',
    'What is the value of 5x - 3 when x = 4?',
    'Solve: 2x + 5 = 13',
    'If y = x² and x = 3, what is y?',
    'Solve: 3(x - 2) = 12',
    'What is x if 2x - 8 = x + 2?',
    'Solve: 3x - 6 = 9',
  ...
  'Geometry Fundamentals': [
    ...
    'What is the circumference of a circle with radius 2? (π = 3.14)',
...
export const getQuestionsForLevel = (grade: string, lesson: string, level: string): string[] => {
  ...
  // Custom handling for Grade 8 Mathematics provided dataset
  if (gradeKey === 'grade8' && G8_TOPICS[lesson]){
    const list = G8_TOPICS[lesson]
...
export const getLessonsForGrade = (grade: string): string[] => {
  const gradeKey = `grade${grade.replace('grade', '')}` 
  if (gradeKey === 'grade8') return Object.keys(G8_TOPICS)
...
export const isGradeSupported = (grade: string): boolean => {
  const gradeKey = `grade${grade.replace('grade', '')}` 
  return gradeKey === 'grade8' || QUESTION_BANK.hasOwnProperty(gradeKey)
}
