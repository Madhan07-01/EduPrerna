      ...
      <div className="max-w-6xl mx-auto p-4">
        ...
        <div className="mt-4 rounded-2xl bg-white shadow-xl border border-gray-200 p-6">
          ...
          <div className="mt-4 rounded-xl bg-gradient-to-r from-purple-100 to-pink-100 border border-purple-200 p-4">
            ...
            <div className="text-lg md:text-xl font-bold text-purple-700">{compoundName}</div>
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3">
              {ALL_ATOMS.map(k => {
                ...
                const showRing = feedback==='correct' && ((activeTargets[k]||0) > 0 || (level>5 && (bonusGoals[bonusIndex]?.targets[k]||0)>0))
                return (
                  <button
                    key={k}
                    onClick={() => setSelectedCounts(prev => ({ ...prev, [k]: (prev[k] || 0) + 1 }))}
                    onContextMenu={(e) => { e.preventDefault(); setSelectedCounts(prev => ({ ...prev, [k]: Math.max(0, (prev[k] || 0) - 1) })) }}
                    className={`relative rounded-xl p-3 text-center border shadow-sm transition hover:shadow-md ${showRing ? 'ring-2 ring-emerald-400' : ''}`}
