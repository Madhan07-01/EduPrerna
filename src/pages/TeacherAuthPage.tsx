...
export default function TeacherAuthPage() {
  const { signInTeacherUsername, loading } = useAuth()
  ...
  const onSubmit = async () => {
    setError(null)
    try {
      await signInTeacherUsername(username, password)
    } catch (e: any) {
  ...
  return (
    <div className="max-w-md mx-auto space-y-4">
      ...
      <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Username" className="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm" />
