 ...
 */
function generateDocumentId(grade: number, lessonTitle: string): string {
 ...
 */
function generatePossibleDocumentIds(grade: number, lessonTitle: string): string[] {
  const ids: string[] = [];
 ...
 */
function generateFallbackDocumentId(grade: number, lessonTitle: string): string {
...
type Subject = keyof typeof lessonsData;

// Helper functions
const getLessonsForSubjectAndGrade = (subject: Subject, grade: Grade): string[] => {
      ...
      for (let chapterIndex = 0; chapterIndex < lessons.length; chapterIndex++) {
        ...
        for (const docId of possibleIds) {
          try {
            ...
            const docSnap = await getDocs(query(collection(db, 'courses/mathematics/lessons'), where('__name__', '==', docRef.path)));
            ...
            console.log(`    ○ Failed to delete with ID ${docId}:`, (error as Error).message);
          }
        }
