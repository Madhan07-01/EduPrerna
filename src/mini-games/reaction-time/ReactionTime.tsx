...
const ReactionTime: React.FC<ReactionTimeProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const step = (t: number) => {
      ...
      setTimeLeft(prev => {
        ...
        if (v === 0) {
          // time up counts as a wrong attempt
          onWrong()
          return levelSpec.timer // reset for next try if lives remain
        }
        return v
      })
  ...
  const handleReact = () => {
    if (levelSpec.type === 'mcq') {
      ...
      const bonus = Math.min(5, timeLeft) // speed bonus up to +5
      return onCorrect(bonus)
    }
    if (levelSpec.type === 'match') {
      // require all matches correct
    ...
    if (levelSpec.type === 'speed') {
      ...
      if (!correct) return onWrong()
      setScore(s => s + 10 + Math.round(Math.min(3, timeLeft / 10))) // small speed bonus per round
      setSelected(null)
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-950 text-white">
      ...
      <div className="max-w-6xl mx-auto pt-4 px-4 flex items-center justify-between">
        ...
        <div className="text-white/90 space-x-6">
          ...
          <span className={`px-2 py-1 rounded ${timeLeft < 10 ? 'bg-red-600/40' : 'bg-white/10'}`}>⏱️ {Math.ceil(timeLeft)}s</span>
        </div>
      </div>
